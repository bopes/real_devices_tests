  ...
  this.execute(this.executeSizzlejs, [this.selector, this.injectedJsCommand()], function (result) {
    ...
    if (result.seens >= JS_SEEN_MAX || self.seenCount >= SEEN_MAX || elapsed > MAX_TIMEOUT) {
      ...
      if (self.seenCount >= SEEN_MAX || result.seens >= JS_SEEN_MAX) {
        ...
        if (result.selectorLength === 1) {
          self.fail("[not visible]", "[visible]", _this.failureMessage + "[SELECTOR_NOT_VISIBLE]");
        } else {
...
Base.prototype.fail = function (actual, expected, failureMessage) {
  ...
  var message = _errorDictionary2.default.format(_util2.default.format((this.isSync ? "[sync mode] " : "") + (failureMessage || this.failureMessage), this.time.totalTime));
